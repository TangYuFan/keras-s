...
#建立token
#读取train一共25000个text样本,统计出现次数排名2000的单词,建立一个2000单词的词典
...
#text文本转为  100个数字的列表  每个数字(即每个单词)转为n维词向量  一个text样本转为100*n的张量  keras不需要自己训练word2voc而是网络中拼接嵌入层
#这一步实际上是将 2000维稀疏向量->32维稠密向量
#为什么是2000维:一个单词根据字典库的onehot编码就是2000维

#拼接网络
model = Sequential()
model.add(Embedding(input_dim=2000,input_length=100,output_dim=32)) #100个单词每个单词转为32维稠密向量 即100*32的张量
model.add(Dropout(0.2)) #防止过拟合
model.add(Flatten()) #展平 100*32=3200个神经元
...
pad_input_seq = sequence.pad_sequences(input_seq,maxlen=100) #保留100个数字
result = model.predict_classes(pad_input_seq) #预测
print('样本集外样本:')
print('result:',result,SentimentDict[result[0][0]])

#如何提高模型预测准确率: 增大样本数据集,同时扩大字典token